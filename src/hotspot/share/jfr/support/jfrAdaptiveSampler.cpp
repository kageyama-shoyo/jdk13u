...
inline bool JfrSamplerWindow::sample() const {
  const size_t ordinal = Atomic::add(static_cast<size_t>(1), &_measured_population_size);
...
void JfrSamplerWindow::initialize(const JfrSamplerParams& params) {
  ...
  if (params.window_duration_ms == 0) {
    Atomic::store(static_cast<int64_t>(0), &_end_ticks);
    return;
  }
  Atomic::store(static_cast<size_t>(0), &_measured_population_size);
  const int64_t end_ticks = now() + millis_to_countertime(params.window_duration_ms);
  Atomic::store(end_ticks, &_end_ticks);
}
